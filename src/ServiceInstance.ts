import { WorkerParentIO, RPCChannel, type DestroyableIoInterface } from "@kunkun/kkrpc"
...
export class ServiceInstance {
  ...
  private rpc?: RPCChannel<object, IService, DestroyableIoInterface>
  ...
  constructor(config: ServiceConfig) {
    ...
    if (!this.config.script && !this.config.worker) {
      throw new Error(`Service '${this.config.name}' must have either 'script' or 'worker' specified`)
    }
  ...
  async start(): Promise<void> {
    ...
    if (!this.worker) {
      if (!this.config.script) {
        throw new Error(`Service '${this.config.name}' has no script or worker specified`)
      }
      // Spawn worker from script
      this.worker = new Worker(new URL(this.config.script, import.meta.url).href, {
        type: "module",
      })
    }
    ...
    this.rpc = new RPCChannel<object, IService, DestroyableIoInterface>(this.io, {})
  ...
  private async runJob(): Promise<void> {
    ...
    if (this.config.timeout && this.status === "running") {
      this.timeoutHandle = setTimeout(() => {
        console.warn(`[Service ${this.config.name}] timeout exceeded, terminating job.`)
        void this.stop()
      }, this.config.timeout)
    }
  ...
  private maybeRestart(exitCode = 1): void {
    ...
    if (shouldRestart) {
      this.restartCount += 1
      console.log(`[Service ${this.config.name}] restarting (#${this.restartCount})...`)
      setTimeout(() => {
        void this.start()
      }, 1000)
    }
